...
import {useGetSeriesQuery, useGetTopPopularShowsQuery} from '@redux/api';
...
export default function Serie() {
  ...
  const {data, isLoading} = useGetTopPopularShowsQuery(1);
  const [page, setPage] = React.useState(1);
  const getMovieQuery = useGetSeriesQuery(page);
  ...
  const _renderTopItems = ({item, index}: {item: Movie; index: number}) => {
    return <TopMovieCard item={item} type="tv" />;
  };

  const _renderItems = ({item, index}: {item: Movie; index: number}) => {
    return <MovieCard item={item} type="tv" />;
  };
  ...
  return (
    <SafeAreaView style={{flex: 1}}>
      <Header text="Series" hasBack={false} hasSearch={true} />
      ...
      <View style={styles.container}>
        ...
        <View style={[styles.topRatedView, {flex: 1}]}>
          <Text style={styles.topText}>Series</Text>
          {getMovieQuery.isLoading ? (
            <View style={{marginTop: 20}}>
              <ActivityIndicator size="large" color={theme.colors.netflixRed} />
            </View>
